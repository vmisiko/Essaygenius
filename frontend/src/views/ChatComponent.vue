  ...
  <div id="curtain" height="100vh">
    <v-card flat height="100%" light>
      <v-toolbar>
        ...
        <v-badge bottom color="green" dot offset-x="10" offset-y="10">
          ...
          >
            <div v-if="chatUsersMap[message.sender] !== loggedInUser.email">
              <span class="ml-2 mb-1">
                <b> {{ chatUsersMap[message.sender] }}: </b>
              </span>
              <br />
              <span class="rounded green white--text px-4 py-1">
                {{ message.text }}
              </span>
            </div>
            ...
            <v-layout v-else>
              <v-spacer></v-spacer>
              <span class="rounded grey lighten-3 black--text px-4 py-2">
                {{ message.text }}
              </span>
            </v-layout>
          </div>
...
export default {
  ...
  methods: {
    ...
    getChatToken() {
      return this.loggedInUser.id < this.selectedUser.id
        ? this.loggedInUser.id + "_" + this.selectedUser.id
        : this.selectedUser.id + "_" + this.loggedInUser.id;
    },
  ...
  created() {
    ...
    instance
      .get(`/messageAPI/message/${senderid}/`)
      .then(res => {
        ...
        this.chatSocket = new WebSocket(
          "ws://localhost:8000" + "/ws/chat/" + this.getChatToken() + "/"
        );
